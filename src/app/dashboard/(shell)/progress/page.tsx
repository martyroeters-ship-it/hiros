    ...
    <div className="flex flex-col gap-5 overflow-y-auto pb-4 pr-1">
      ...
      <div className="rounded-[24px] bg-white p-5 shadow-[0_2px_16px_rgba(31,51,41,0.05)]">
        <div className="flex items-baseline justify-between">
          <div>
            ...
            <p className="mt-0.5 text-[12px] text-[#8a9288]">
              {loading ? "Loading…" : photos.length ? `${photos.length} photo${photos.length === 1 ? "" : "s"} on file` : "No photos yet"}
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="mt-4 flex flex-col items-center justify-center rounded-[16px] border border-dashed border-[#e4e0d8] bg-[#faf9f6] px-4 py-10 text-center">
            <p className="text-[14px] font-semibold text-[#3d4540]">No photos yet</p>
            <p className="mt-1 max-w-[36ch] text-[13px] leading-relaxed text-[#8a9288]">
              Upload photos so your physician can follow your progress. We only show dates that actually happened.
            </p>
            <Link href="/dashboard/photos" className="mt-4 rounded-full bg-[#1f4033] px-4 py-2 text-[13px] font-semibold text-white">
              Upload photos
            </Link>
          </div>
        ) : (
