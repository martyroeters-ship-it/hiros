...
export default function DoctorPortalPage() {
  const [sidebarTab, setSidebarTab] = useState<"cases" | "patients" | "agenda" | "messages" | "settings">("cases");
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#f4f5f3] font-[var(--font-dm-sans)] text-[#2b2a28]">
      {/* Sidebar */}
        ...
        <div className="flex flex-1 flex-col items-center gap-2 py-8">
          ...
          <button
            ...
            className={`flex h-14 w-14 items-center justify-center rounded-[12px] transition-colors ${
              sidebarTab === "cases" ? "bg-white/20" : "hover:bg-white/10"
            }`}
            title="Cases"
          >
          ...
          <button
            ...
            className={`flex h-14 w-14 items-center justify-center rounded-[12px] transition-colors ${
              sidebarTab === "patients" ? "bg-white/20" : "hover:bg-white/10"
            }`}
            title="Patients"
          >
            <svg viewBox="0 0 24 24" fill="none" className="h-6 w-6 text-white" stroke="currentColor" strokeWidth="1.5">
          ...
          <button
            ...
            className={`flex h-14 w-14 items-center justify-center rounded-[12px] transition-colors ${
              sidebarTab === "agenda" ? "bg-white/20" : "hover:bg-white/10"
            }`}
            title="Agenda"
          >
            <svg viewBox="0 0 24 24" fill="none" className="h-6 w-6 text-white" stroke="currentColor" strokeWidth="1.5">
          ...
          <button
            ...
            className={`flex h-14 w-14 items-center justify-center rounded-[12px] transition-colors ${
              sidebarTab === "messages" ? "bg-white/20" : "hover:bg-white/10"
            }`}
            title="Messages"
          >
            <svg viewBox="0 0 24 24" fill="none" className="h-6 w-6 text-white" stroke="currentColor" strokeWidth="1.5">
      ...
      {/* Main Content */}
      <div className="flex-1 lg:ml-[90px]">
        {/* Top bar */}
        <header className="sticky top-0 z-20 border-b border-black/5 bg-white/85 backdrop-blur-md">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-3.5">
          <div className="flex items-center gap-3">
            <div className="leading-tight">
              <p className="font-title text-[18px] font-semibold tracking-[-0.03em] text-[#1f241b]">Doctor Dashboard</p>
              <p className="text-[13px] font-medium text-black/45">Dr. Bülent (Demo)</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-1.5 rounded-full px-2.5 py-2 text-[13px] font-medium text-black/55 hover:bg-black/[0.04]">
              <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4" aria-hidden="true">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            placeholder="Search by Case ID (e.g. HIROS-MJWZE3A)"
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 rounded-full px-4 py-2 text-[13.5px] font-semibold transition ${
                    active
          ...
          <div className="flex items-center gap-2.5">
            ...
            {filterMenuOpen && (
              <>
                ...
                <div className="absolute left-0 top-full z-20 mt-2 w-48 overflow-hidden rounded-[14px] border border-black/10 bg-white shadow-[0_8px_24px_rgba(0,0,0,0.12)]">
                  {[
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-full border border-black/10 bg-white px-4 py-2 text-[13.5px] font-semibold text-[#2b2a28] shadow-[0_1px_2px_rgba(0,0,0,0.04)] hover:bg-black/[0.02]"
            >
              <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4 text-black/50" aria-hidden="true">
          ...
          {visibleCases.map((c) => {
            ...
            return (
              <Link
                key={c.id}
                href={`/doctor/${c.id}`}
                className="group relative flex w-full items-center justify-between gap-4 overflow-hidden rounded-[16px] border border-black/[0.06] bg-white px-5 py-4 text-left shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition hover:shadow-[0_8px_24px_rgba(0,0,0,0.07)]"
              >
                <span className={`absolute left-0 top-3 bottom-3 w-1.5 rounded-full ${styles.bar}`} />
                <div className="pl-3">
                  <div className="flex flex-wrap items-center gap-2.5">
                    <span className="font-[var(--font-geist-mono)] text-[14px] font-semibold tracking-[-0.01em] text-[#1f241b]">
                      {c.id}
                    </span>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-4 text-[12.5px] font-medium text-black/50">
